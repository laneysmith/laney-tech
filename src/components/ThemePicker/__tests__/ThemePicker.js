...
describe('ThemePicker', () => {
  ...
  it('should call setTheme with the correct values', () => {
    const { container, rerender } = render(<ThemePicker {...PROPS} />);
    const firstThemeButton = container.querySelector(`#${THEME_LIST[0]}-theme-btn`);
    const secondThemeButton = container.querySelector(`#${THEME_LIST[1]}-theme-btn`);
  ...
  it('should disable currently selected theme button', () => {
    ...
    const firstThemeButton = container.querySelector(`#${THEME_LIST[0]}-theme-btn`);
    const secondThemeButton = container.querySelector(`#${THEME_LIST[1]}-theme-btn`);
    const thirdThemeButton = container.querySelector(`#${THEME_LIST[2]}-theme-btn`);
